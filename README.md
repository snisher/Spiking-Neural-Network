...
While temporal encoding in spiking networks brings artificial computation closer to biological networks, there are other differences that need to be addressed. Artificial spiking networks still have a feed forward, "fully" connected architecture where each layer is only connected to the layer directly before and after it. Biological networks, on the other hand, do not have a linear architecture and have neurons connecting to previous layers, forward layers, etc.
